...
describe(ContactModal.name, () => {
    ...
    it("should render", () => {
        ...
        expect(github_anchor.getAttribute("href")).toBe("https://github.com/kruhlmann");

        const mail_anchor = query_select(container, "a[title='Email']");
        expect(mail_anchor.getAttribute("href")).toBe("mailto:andreas@kruhlmann.dev");

        const linkedin_anchor = query_select(container, "a[title='LinkedIn']");
        expect(linkedin_anchor.getAttribute("href")).toBe("https://linkedin.com/in/andreaskruhlmann");

        const discord_anchor = query_select(container, "a[title='Discord']");
        expect(discord_anchor.getAttribute("href")).toBe("https://discord.gg/Y5eA3dW");

        const gitlab_anchor = query_select(container, "a[title='GitLab']");
        expect(gitlab_anchor.getAttribute("href")).toBe("https://gitlab.com/Kruhlmann");
